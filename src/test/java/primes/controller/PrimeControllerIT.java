...
public class PrimeControllerIT {
    ...
    public void setUp() throws Exception {
        ...
        when(primeService.sieveOfEratosthenes(10))
                .thenReturn(new Result(10, Arrays.asList(2, 3, 5, 7)));
    }
    ...
    public void should_return_xml_response() throws Exception {
        mockMvc.perform(get("/primes/10").accept(MediaType.APPLICATION_XML))
                .andExpect(status().isOk()).andExpect(content().contentType("application/xml"))
                .andExpect(xpath("//result/@initial").string("10"))
                .andExpect(xpath("(//prime)[1]").string("2"))
                .andExpect(xpath("(//prime)[2]").string("3"))
                .andExpect(xpath("(//prime)[3]").string("5"))
                .andExpect(xpath("(//prime)[4]").string("7"));
    }
